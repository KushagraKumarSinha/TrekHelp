...
function Ohiopyle() {
    ...
    const handleOhiopyle = async () => {
        const ReturnValue = await axios.get('http://127.0.0.1:8000/WorldsEndGet');
        setDetails3(ReturnValue.data);

        const ReturnVal = await axios.get('http://127.0.0.1:8000/WorldsEndMessageGet');
    ...
    const handleTextSend = async() => {
        const response = await axios.post("http://127.0.0.1:8000/WorldsEndMessagePost", {
